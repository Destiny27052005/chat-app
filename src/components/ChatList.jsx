...
export default function ChatList({
        ...
        if (lastMsg.file) {
            return (
                <span className="inline-flex items-center gap-1 text-indigo-600 font-medium">
                    ...
                    <span>{lastMsg.file.name || 'Attachment'}</span>
                </span>
            );
        }
    ...
    // Resolve dynamic avatar/name for direct 1-to-1 conversations
    const getRoomDisplayDetails = (room) => {
        ...
        return {
            ...
            isOnline: Boolean(otherMember?.isOnline || room.isOnline),
        };
    };
    ...
    return (
        <aside
            className={`w-full md:w-80 bg-white border-r border-slate-100 flex flex-col h-full shrink-0 ${activeRoom ? 'hidden md:flex' : 'flex'
                }`}
        >
            <div className="w-80 bg-white border-r border-slate-100 flex flex-col shrink-0 h-full select-none">
                {/* Header & Filter Controls */}
                <div className="p-4 border-b border-slate-100 shrink-0 space-y-3">
                    <div className="flex items-center justify-between">
                        <h3 className="font-bold text-slate-800 text-base">Messages</h3>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-7 py-2 bg-slate-50 border border-slate-200/80 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 transition text-slate-700 placeholder:text-slate-400"
                        />
                        {searchQuery && (
                            <button
                                type="button"
                    ...
                    <div className="flex items-center gap-1.5 pt-0.5">
                        {['all', 'direct', 'groups'].map((type) => (
                            <button
                                ...
                                className={`px-3 py-1 rounded-lg text-xs font-semibold capitalize transition ${filterType === type
                                    ? 'bg-indigo-600 text-white shadow-xs'
                                    : 'bg-slate-100 hover:bg-slate-200/70 text-slate-500'
                                    }`}
                            >
                                {type}
                            </button>
                ...
                <div className="flex-1 overflow-y-auto divide-y divide-slate-50">
                    {isLoading ? (
                        <div className="h-full flex flex-col items-center justify-center p-8 text-center text-xs text-slate-400 gap-2">
                        ...
                        filteredRooms.map((room) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`flex items-center gap-3 p-3.5 cursor-pointer transition ${isSelected
